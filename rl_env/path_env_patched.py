...
class RlGame(gym.Env):
    ...
    def reset(self,action):#reset的仅是环境状态，
        ...
        # ---- Minimal fix: apply actions first, then compute rewards on the updated state ----
        for _i in range(self.hero_num):
            self.hero['hero' + str(_i)].update(action[_i], self.Render)
        for _j in range(self.enemy_num):
            self.enemy['enemy' + str(_j)].update(action[self.hero_num + _j], self.Render)
        # -----------------------------------------------------------------------------
