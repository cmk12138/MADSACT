...
class RlGame(gym.Env):
    def __init__(self, n=None, m=None, render=False, config=None):
        """Create the environment.

        Parameters
        ----------
        n : int | None
            Number of friendly UAVs (heroes). If None, will be read from:
            1) config['hero_num'] (if provided)
            2) environment variable HERO_NUM
            3) default = 1
        m : int | None
            Number of enemy UAVs (followers). If None, will be read from:
            1) config['enemy_num'] (if provided)
            2) environment variable ENEMY_NUM
            3) default = 1

        Tip: you can launch experiments without editing code by setting env vars, e.g.
             HERO_NUM=3 ENEMY_NUM=5 python train.py
        """
        import os as _os
        if config is None:
            config = {}
        if n is None:
            n = int(config.get('hero_num', _os.getenv('HERO_NUM', 1)))
        if m is None:
            m = int(config.get('enemy_num', _os.getenv('ENEMY_NUM', 1)))
        # basic validation
        if n <= 0 or m <= 0:
            raise ValueError(f"hero_num and enemy_num must be positive, got n={n}, m={m}")
        self.hero_num = n
        self.enemy_num = m
        self.obstacle_num=1
        self.goal_num=1
        ...
        for i in range(self.hero_num+self.enemy_num):
            ...
            if i==0:#leader
                ...
                if self.hero['hero' + str(i)].posy >= C.ENEMY_AREA_HEIGHT:
                    ...
                    edge_r[i] = -1
                if 0 < dis_1_agent_0_to_1 < 50 :                                                                                      #——————————————————————————————————————————————————————————————————————————————————————训练用
                ###################################################################################################################
                #if 0 < dis_1_agent_0_to_1 < 50 and dis_1_agent_0_to_2 < 50 and dis_1_agent_0_to_3 < 50 and dis_1_agent_0_to_4 < 50:  #——————————————————————————————————————————————————————————————————————————————————————测试用
                #if 0 < dis_1_agent_0_to_1 < 50 and dis_1_agent_0_to_2:
                #if 0 < dis_1_agent_0_to_1 < 50 and dis_1_agent_0_to_2 < 50 and dis_1_agent_0_to_3 < 50:
                    #follow_r0=0
                    #self.team_counter += 1
                ...
                #     r = 1.0
                    ...
                    # ChatGPT Modified Here
                    d_goal = float(dis_1_goal[i])
                    progress = np.clip(self.prev_d_goal_leader - d_goal, -progress_clip, progress_clip)
                ...
                # ChatGPT Modified Here
                speed_r_f = 0.0
                act_pen = -w_act * (abs(action[i][0]) + abs(action[i][1])) #（2）动作幅度惩罚
