...
shoplistfile = 'E:/PycharmProjects/code/UAV-path-planning-main/Multi-UAVs path planning/path planning\MADDPG'  #保存文件数据所在文件的文件名
shoplistfile_test = 'E:/PycharmProjects/code/UAV-path-planning-main/Multi-UAVs path planning/path planning\MADDPG_compare'  #保存文件数据所在文件的文件名
...
def run(env):
    if Switch==0:
        ...
        for k in range(TRAIN_NUM):
            ...
            for episode in range(EPIOSDE_ALL):
                ...
                if episode % 50 == 0 and episode > 200:#保存神经网络参数
                    ...
                    torch.save(save_data, "E:/PycharmProjects/code/UAV-path-planning-main/Multi-UAVs path planning/path planning\Path_DDPG_actor_new.pth")
                    save_data = {'net': actors[1].actor_estimate_eval.state_dict(), 'opt': actors[1].optimizer.state_dict()}
                    torch.save(save_data, "E:/PycharmProjects/code/UAV-path-planning-main/Multi-UAVs path planning/path planning\Path_DDPG_actor_1_new.pth")
        ...
        print('MADDPG测试中...')
        aa = Actor()
        checkpoint_aa = torch.load("E:/PycharmProjects/code/UAV-path-planning-main/Multi-UAVs path planning/path planning/Path_DDPG_actor_new.pth")
        aa.actor_estimate_eval.load_state_dict(checkpoint_aa['net'])
        bb = Actor()
        checkpoint_bb = torch.load("E:/PycharmProjects/code/UAV-path-planning-main/Multi-UAVs path planning/path planning/Path_DDPG_actor_1_new.pth")
