...
# ---- Reward curve smoothing (for plotting only) ----
# Uses smoothing.py if available; falls back to EMA if gaussian filter is missing.
try:
    from smoothing import gaussian_filter1d  # type: ignore
except Exception:
...
def smooth_curve(
    ...
    """Smooth a 1D curve for visualization only (does NOT affect training)."""
    ...
    for _ in range(passes):
        ...
        if method_local == "gaussian":
            if gaussian_filter1d is not None and sigma and sigma > 0:
                y = gaussian_filter1d(y, float(sigma), mode="nearest").astype(np.float32)
                continue
            method_local = "ema"
        if method_local in ("ema", "exponential"):
            ...
            y = y_next
            continue
        if method_local in ("uniform", "moving_average", "ma") and window and window > 1:
            w = int(window)
            kernel = np.ones((w,), dtype=np.float32) / float(w)
            pad = w // 2
            yp = np.pad(y, (pad, w - 1 - pad), mode="edge")
            y = np.convolve(yp, kernel, mode="valid").astype(np.float32)
            continue
        return y
    return y
...
def plot_reward_curves(leader_ep: np.ndarray, follower_ep: Optional[np.ndarray], total_ep: np.ndarray, save_path: Optional[str] = None):
    """
    Simple reward visualization: total/leader/follower curves with optional save.
    """
    try:
        plt.figure(figsize=(8, 4), dpi=150)
        plt.plot(total_ep, label="Total")
        plt.plot(leader_ep, label="Leader")
        if follower_ep is not None and len(follower_ep) > 0:
            if not np.all(np.isnan(follower_ep)):
                plt.plot(follower_ep, label="Follower")
        plt.xlabel("Episode")
        plt.ylabel("Reward")
        plt.legend()
        plt.tight_layout()
        ...
        if save_path:
            ...
            print(f"Saved reward curves -> {save_path}", flush=True)
        
        plt.close()  # 不要用 plt.show()，用 Agg 后端时会出问题
        
    except Exception as e:
        print(f"plot_reward_curves error: {e}", flush=True)
        import traceback
        traceback.print_exc()


def plot_rewards_like_main(leader_ep, follower_ep, save_dir: str):
    """
    Plot reward curves in the same style as main_SAC_dsact.py:
    - Figure 1: Total reward (sum of leader+follower) + band (mean±std*0.95)
    - Figure 2: Leader reward + band
    - Figure 3: Follower reward + band
    Here we usually have a single run; std will be 0, but we keep the same structure.
    """
    leader_ep = np.asarray(leader_ep, dtype=np.float32)
    follower_ep = np.asarray(follower_ep, dtype=np.float32)
    total_sum = leader_ep + follower_ep
    total_mean = 0.5 * (leader_ep + follower_ep)

    # Shape as [runs, episodes] like main_SAC_dsact.py
    all_ep_r = total_sum[None, :]
    all_ep_r0 = leader_ep[None, :]
    all_ep_r1 = follower_ep[None, :]

    all_ep_r_mean = np.mean(all_ep_r, axis=0)
    all_ep_r_std = np.std(all_ep_r, axis=0)
    all_ep_L_mean = np.mean(all_ep_r0, axis=0)
    all_ep_L_std = np.std(all_ep_r0, axis=0)
    all_ep_F_mean = np.mean(all_ep_r1, axis=0)
    all_ep_F_std = np.std(all_ep_r1, axis=0)
    ...
    xs = np.arange(len(all_ep_r_mean))

    # Fig 1: total(sum) + mean line
    plt.figure(1, figsize=(8, 4), dpi=150)
    plt.margins(x=0)
    plt.plot(xs, all_ep_r_mean, label='Total(sum)', color='#e75840')
    plt.fill_between(xs, all_ep_r_max, all_ep_r_min, alpha=0.6, facecolor='#e75840')
    plt.plot(xs, total_mean, label='Total(mean)', color='#3a7bd5')
    plt.xlabel('Episode')
    ...
    plt.margins(x=0)
    plt.plot(xs, all_ep_L_mean, label='Leader', color='#e75840')
    plt.fill_between(xs, all_ep_L_max, all_ep_L_min, alpha=0.6, facecolor='#e75840')
    plt.xlabel('Episode')
    ...
    plt.margins(x=0)
    plt.plot(xs, all_ep_F_mean, label='Follower', color='#e75840')
    plt.fill_between(xs, all_ep_F_max, all_ep_F_min, alpha=0.6, facecolor='#e75840')
    plt.xlabel('Episode')
    plt.ylabel('Follower reward')

    plt.legend()
    os.makedirs(save_dir, exist_ok=True)
    plt.figure(1); plt.savefig(os.path.join(save_dir, "reward_total.png"), dpi=150, bbox_inches="tight")
    plt.figure(2); plt.savefig(os.path.join(save_dir, "reward_leader.png"), dpi=150, bbox_inches="tight")
    plt.figure(3); plt.savefig(os.path.join(save_dir, "reward_follower.png"), dpi=150, bbox_inches="tight")

    print(f"Saved reward figures -> {os.path.abspath(save_dir)}", flush=True)
    plt.show()
...
class Config:
    ...
    # misc
    SEED: int = 43
    SAVE_DIR: str = "./runs/r1"
    SAVE_EVERY_EP: int = 50

    # reward curve smoothing (plot only)
    SMOOTH_METHOD: str = "ema"         # ema | gaussian | uniform | none
    SMOOTH_SIGMA: float = 5.0          # for gaussian
    SMOOTH_ALPHA: float = 0.85         # for ema (higher = smoother)
    SMOOTH_WINDOW: int = 10            # for uniform moving average
    SMOOTH_PASSES: int = 2             # repeat smoothing passes for extra smoothness
...
def train(cfg: Config):
    ...
    for ep in range(cfg.EP_MAX):
        ...
        for t in range(cfg.EP_LEN):
            ...
            if buf.size >= max(cfg.START_STEPS, cfg.BATCH) and (global_step % cfg.UPDATE_EVERY == 0):
                ...
                for i in range(num_agents):
                    ...
                    ratio1 = torch.ones_like(q1_m)
                    ratio2 = torch.ones_like(q2_m)
        ...
        # Leader/Follower convention: leader=agent 0, follower=agent 1 (if exists)
        leader_idx = 0
        follower_idx = 1 if num_agents > 1 else None
        leader_ret = float(ep_ret[leader_idx])
        follower_ret = float(ep_ret[follower_idx]) if follower_idx is not None else 0.0
        total_ret = leader_ret + follower_ret
        leader_returns.append(leader_ret)
        follower_returns.append(follower_ret if follower_idx is not None else np.nan)
    ...
    # ---- Plot reward curves (leader/follower/total) ----
    print("\n" + "="*50, flush=True)
    print("开始绘制奖励曲线...", flush=True)
    print(f"Leader rewards 数量: {len(leader_returns)}", flush=True)
    print(f"Follower rewards 数量: {len(follower_returns)}", flush=True)
    print(f"Total rewards 数量: {len(total_returns)}", flush=True)
    
    fig_path = os.path.join(cfg.SAVE_DIR, "reward_curves.png")
    print(f"图片保存路径: {os.path.abspath(fig_path)}", flush=True)
    
    try:
        # 准备数据
        leader_arr = np.array(leader_returns, dtype=np.float32)
        total_arr = np.array(total_returns, dtype=np.float32)
        # Smooth curves for visualization (does NOT change training)
        method = (getattr(cfg, 'SMOOTH_METHOD', 'gaussian') or '').lower()
        if method == "gaussian" and gaussian_filter1d is None:
            method = "ema"
        passes = getattr(cfg, 'SMOOTH_PASSES', 1)
        if method and method != 'none':
            leader_smooth = smooth_curve(
                leader_arr,
                method=method,
            ...
            leader_smooth, total_smooth = leader_arr, total_arr
        follower_arr = None if (num_agents <= 1) else np.array(follower_returns, dtype=np.float32)
        if follower_arr is not None and method and method != 'none':
            follower_smooth = smooth_curve(
                follower_arr,
                method=method,
                sigma=getattr(cfg, 'SMOOTH_SIGMA', 5.0),
                alpha=getattr(cfg, 'SMOOTH_ALPHA', 0.1),
        ...
        print(f"Leader 数据前5个: {leader_arr[:5]}", flush=True)
        print(f"Total 数据前5个: {total_arr[:5]}", flush=True)
        
        # 创建图形
        plt.figure(figsize=(10, 5), dpi=150)
        # plot smoothed curves (primary)
        ...
        print(f"✓ 奖励曲线已保存!", flush=True)
        
        # 验证文件是否存在
        if os.path.exists(fig_path):
            file_size = os.path.getsize(fig_path)
            print(f"✓ 文件已创建: {fig_path} (大小: {file_size} bytes)", flush=True)
        else:
            print(f"✗ 警告: 文件未创建!", flush=True)
        
        plt.close()  # 关闭图形，释放内存

        # 额外保存三张单独曲线图片
        def save_single_curve(data_arr, label, color, filename, ylabel):
            plt.figure(figsize=(10, 5), dpi=150)
            plt.plot(data_arr, label=label, linewidth=1.5, color=color)
            plt.xlabel("Episode")
            plt.ylabel(ylabel)
            ...
            if os.path.exists(save_path):
                ...
                print(f"? 单独曲线已保存: {save_path} (大小: {file_size} bytes)", flush=True)
            else:
                print(f"? 警告: 单独曲线未创建: {save_path}", flush=True)

        save_single_curve(total_smooth, "Total(smooth)", "green", "reward_total_only.png", "Total Reward")
        save_single_curve(leader_smooth, "Leader(smooth)", "blue", "reward_leader_only.png", "Leader Reward")
        if follower_smooth is not None:
            save_single_curve(follower_smooth, "Follower(smooth)", "red", "reward_follower_only.png", "Follower Reward")
        else:
            print("? 未检测到 follower 数据，跳过 follower 单独曲线保存", flush=True)
