...
# Default behavior:
#   - training: dt=1.0
#   - evaluation/test: dt=0.1
# Override priority:
#   1) environment variable RL_DT (float)
#   2) environment variable RL_MODE in {test, eval, evaluation, inference} -> 0.1
#      otherwise -> 1.0
def _resolve_dt():
    v = os.getenv('RL_DT', '').strip()
    if v:
        try:
            return float(v)
        except ValueError:
            pass
    mode = os.getenv('RL_MODE', 'train').strip().lower()
    if mode in {'test', 'eval', 'evaluation', 'inference'}:
        return 0.1
    return 1.0
