...
import matplotlib
# Use a GUI backend on Windows so plt.show() opens a window.
# Fall back to a non-GUI backend on headless Linux (no DISPLAY).
if os.name == "nt":
    try:
...
def plot_reward_curves(leader_rewards, follower_rewards, total_rewards, save_path: str = None):
    """
    Plot leader, follower, and total (= leader + follower) episode reward curves.
    - leader index defaults to 0
    - follower index defaults to 1 (if exists)
    """
    xs = np.arange(len(total_rewards))
    plt.figure()
    plt.plot(xs, leader_rewards, label="Leader reward")
    if follower_rewards is not None:
        plt.plot(xs, follower_rewards, label="Follower reward")
    plt.plot(xs, total_rewards, label="Total reward (Leader+Follower)")
    plt.xlabel("Episode")
...
def train(cfg: Config):
    ...
    try:
        ...
        # follower_returns may contain NaN if no follower; pass None in that case
        follower_series = None if (num_agents <= 1) else np.array(follower_returns, dtype=np.float32)
        plot_reward_curves(
            np.array(leader_returns, dtype=np.float32),
            follower_series,
            np.array(total_returns, dtype=np.float32),
            save_path=fig_path,
        )
    except Exception as e:
        print(f"[WARN] Failed to plot rewards: {e}", flush=True)
