...
def _load_module_from_path(mod_name: str, file_path: str):
    """Load a module from an explicit file path (works even if not in PYTHONPATH)."""
...
def apply_env_overrides(dt: Optional[float] = None, use_dynamic_agents: bool = True, base_dir: Optional[str] = None):
    """    Ensure this test run uses:
      - player_dynamic_dt.py (so dt is controlled by env var RL_DT / RL_MODE)
      - path_env_dynamic_agents.py (so agent counts can be configured without editing env code)

    This function works in two layouts:
      A) Files are in package paths:
         - rl_env/path_env_dynamic_agents.py
         - assignment/components/player_dynamic_dt.py
      B) Files are next to this test script (same folder).

    Mechanism: load modules and alias them in sys.modules so existing imports keep working.
    """
    if base_dir is None:
        base_dir = os.path.dirname(os.path.abspath(__file__))

    # --- Configure dt (test default) ---
    if dt is not None:
        os.environ["RL_DT"] = str(float(dt))
    else:
        # If user didn't specify, default test dt=0.1
        os.environ.setdefault("RL_DT", "0.1")

    # --- Patch player module import target: assignment.components.player ---
    player_paths = [
        os.path.join(base_dir, "assignment", "components", "player_dynamic_dt.py"),
        os.path.join(base_dir, "player_dynamic_dt.py"),
    ]
    player_mod = None
    for p in player_paths:
        if os.path.exists(p):
            player_mod = _load_module_from_path("_player_dynamic_dt", p)
            break
    if player_mod is None:
        # fallback: try regular import if user placed it in package
        try:
            player_mod = importlib.import_module("assignment.components.player_dynamic_dt")
        except Exception as e:
            raise FileNotFoundError(
                "player_dynamic_dt.py not found. Put it at ./assignment/components/player_dynamic_dt.py "
                "or in the same folder as this test script."
            ) from e

    # Alias to the canonical import that env uses
    sys.modules["assignment.components.player"] = player_mod

    # --- Patch env module import target: rl_env.path_env (optional) ---
    if use_dynamic_agents:
        env_paths = [
            os.path.join(base_dir, "rl_env", "path_env_dynamic_agents.py"),
            os.path.join(base_dir, "path_env_dynamic_agents.py"),
        ]
        env_mod = None
        for p in env_paths:
            if os.path.exists(p):
                env_mod = _load_module_from_path("_path_env_dynamic_agents", p)
                break
        if env_mod is None:
            # fallback: try regular import if user placed it in package
            try:
                env_mod = importlib.import_module("rl_env.path_env_dynamic_agents")
            except Exception as e:
                raise FileNotFoundError(
                    "path_env_dynamic_agents.py not found. Put it at ./rl_env/path_env_dynamic_agents.py "
                    "or in the same folder as this test script."
                ) from e

        # Alias to match training scripts that import rl_env.path_env
        sys.modules["rl_env.path_env"] = env_mod
...
def main():
    ...
    parser.add_argument("--dt", type=float, default=0.1, help="Simulation timestep dt for test. Will set env var RL_DT before importing env/player (default: 0.1)")
    parser.add_argument("--no_dynamic_agents", action="store_true", help="Disable path_env_dynamic_agents override (use default rl_env.path_env)")
    ...
    # Apply overrides so this test uses player_dynamic_dt.py (dt via RL_DT)
    # and path_env_dynamic_agents.py (agent count configurable).
    apply_env_overrides(dt=args.dt, use_dynamic_agents=(not args.no_dynamic_agents))
